...
#Begin test for valid number and thickness. Prints warning relevant to currenet development features
...
while (begLayerNumTest == True):
	try:
		...
		if (begLayerNum <= 0):
			print ("For CURA we need to start with at least LAYER:0. S3D usually greater than 0.")
			raftCheckTest = True
			while (raftCheckTest): 
				buildingOnRaft = input("Printing on a raft? (Y/N)")   #Chosing Y or N we need to ensure valid input for both cases and incorrect input.
				buildingOnRaft = buildingOnRaft[0].upper()
				if (buildingOnRaft == "Y"):
					try:
						numRaftLayers = int(input('How many raft layers? ')) #Use this to check numbmer of positive layers in S3D otherwise we don't really need to know for CURA
					except:
						print ("That doesn't seem to be a valid number.")
					else:
						raftCheckTest = False
						begLayerNumTest = False
				elif (buidlingOnRaft == "N"):
					raftCheckTest = False
					begLayerNumTest = False
				else:
    ...
    try:
        ...
        if (liftHeight >= machHeight):
            ...
            if (liftHeight <= fabricThick) and (begLayerNum == 0):
                print ("Need to lift the head at least higher than the fabric layer thickness of ", fabricThick, " .")
                liftHeightTest = True
            else:
...
firstLayer = ';LAYER:' + str(begLayerNum)   #Declaring the line to search for to beginning inserting material
#S3D first layer format: "; layer 5, Z = 1.917"
